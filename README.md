...
- All pages that need task data read from these keys using shared helper functions in `main-8.js`, so the same data is available on:
...
### 2.1 Tasks Page (`tasks.html` + `tasks-9.js`)
...
### 2.2 Latest Activity Section (Home, `index.html` + `main-8.js`)
...
- In `tasks-9.js`, whenever a task is created, updated, deleted, or completed, the code calls:
...
- On `index.html`, the “Latest Activity” section is rendered using a small block of logic in `main-8.js`:
...
### 2.3 Personal Page (Analytics Page `analytics.html` + `analytics-10.js`)
...
- CSS (`styles.css`).
- JavaScript (`main-8.js`, `tasks-9.js`, `analytics-10.js`, `contact-7.js`).
- Assets such as the TaskForge logo and any other images.

---

## 4. Lighthouse / Auditing Results
...
> In the project, the two screenshots above should be saved (for example) as:
>
> - `docs/lighthouse-mobile.png`
> - `docs/lighthouse-desktop.png`
>
> and committed with the README so the image links work on GitHub.
...
### 5.2 Managing state without a backend

Another challenge was keeping task data and analytics in sync across multiple pages without a server. I solved this by:

- Choosing `localStorage` as the single source of truth for:
  - Task list
  - Latest activity
- Implementing helper functions (`getStoredTasks`, `getStoredActivity`, `saveTasks`) and reusing them in multiple JS files so I didn’t duplicate logic.
- Designing the Tasks page as the primary place where tasks are created and modified, and all other pages (Home, Analytics) only **read** from storage.
...
This separation made it easier to tune the appearance without changing JavaScript.
...
- Keep each JS file focused:
  - `main-8.js` → shared features (dark mode, footer year, home counters, latest activity, quotes).
  - `tasks-9.js` → task CRUD, filters, and summary.
  - `analytics-10.js` → analytics calculations and charts.
  - `contact-7.js` → contact form validation and confirmation modal.
...
### 5.5 Working iteratively with Git and GitHub

Using Git and GitHub helped keep the project organized:

- I committed in logical steps (layout, tasks CRUD, filters, analytics, styling, dark mode, etc.).
- This made it easier to roll back or compare changes when something broke (for example, when dark mode initially interfered with Bootstrap’s default colors).
- The GitHub repository is also the place where I store this `README.md` report so everything is in one place.
...
1. Clone the repository:
   ...
   ```bash
   git clone https://github.com/FireShadowGR/InternetProgramming.git
...
3. Use the navigation bar to access:
   - **Home** – overview, latest activity, testimonials, mini analytics.
   - **Tasks** – create and manage tasks.
   - **Analytics** – detailed stats and charts.
   - **About** – app description and team.
   - **Contact** – contact form and map.

4. Use the dark mode toggle in the navbar to switch themes. Since data is stored in `localStorage`, tasks and activity will persist between reloads in the same browser.
